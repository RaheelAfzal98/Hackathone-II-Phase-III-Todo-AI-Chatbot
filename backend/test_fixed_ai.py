import os
import sys
import requests
import json
from datetime import datetime

# Set the correct environment variables
os.environ['DATABASE_URL'] = 'postgresql://neondb_owner:npg_JDbCa6ELqGQ2@ep-withered-shape-a7bd9hif-pooler.ap-southeast-2.aws.neon.tech/neondb?sslmode=require&channel_binding=require'
os.environ['NEON_DB_URL'] = 'postgresql://neondb_owner:npg_JDbCa6ELqGQ2@ep-withered-shape-a7bd9hif-pooler.ap-southeast-2.aws.neon.tech/neondb?sslmode=require&channel_binding=require'
os.environ['BETTER_AUTH_SECRET'] = 'pohwuyqoVn683bmFDoVzmtQq50Zn3bFV'
os.environ['SECRET_KEY'] = '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'
os.environ['OPENAI_API_KEY'] = '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'
os.environ['OPEN_ROUTER_API_KEY'] = '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'

print("=== Testing Fixed AI Operations ===")

# Create a new user for this test
timestamp = str(int(datetime.now().timestamp()))
unique_email = f"fixed_test_{timestamp}@example.com"

print(f"Creating test user: {unique_email}")

# Register a new user
register_data = {
    "email": unique_email,
    "password": "password123",
    "confirm_password": "password123",
    "name": f"Fixed Test {timestamp}"
}

try:
    response = requests.post("http://localhost:8000/api/v1/register", json=register_data)
    print(f"Registration response: {response.status_code}")
    
    if response.status_code == 200:
        result = response.json()
        print("[OK] Registration successful!")
        token = result.get('token')
        user_id = result.get('id')
        print(f"User ID: {user_id}")
        
        headers = {
            "Authorization": f"Bearer {token}",
            "Content-Type": "application/json"
        }
        
        print(f"\n=== Testing the previously problematic command ===")
        
        # Add a task first so we have something to complete
        add_msg = {"message": "Add a task to buy groceries"}
        response = requests.post(f"http://localhost:8000/api/{user_id}/chat", json=add_msg, headers=headers)
        if response.status_code == 200:
            result = response.json()
            print(f"[OK] Added task: {result.get('response', 'No response')}")
        else:
            print(f"[ERROR] Failed to add task: {response.text}")
        
        # Get the task ID
        tasks_response = requests.get(f"http://localhost:8000/api/v1/users/{user_id}/tasks", headers=headers)
        real_task_id = None
        if tasks_response.status_code == 200:
            tasks = tasks_response.json()
            if tasks:
                real_task_id = tasks[0]['id']
                print(f"[OK] Got real task ID: {real_task_id[:8]}...")
        
        print(f"\n--- Testing problematic command: 'Complete the task to buy groceries' ---")
        # This was the problematic command that caused the error
        problematic_msg = {"message": "Complete the task to buy groceries"}
        response = requests.post(f"http://localhost:8000/api/{user_id}/chat", json=problematic_msg, headers=headers)
        print(f"Response status: {response.status_code}")
        if response.status_code == 200:
            result = response.json()
            response_text = result.get('response', 'No response')
            print(f"Response: {response_text}")
            
            # Check if the error still occurs
            if "Tool execution failed" in response_text and "Failed to complete task:" in response_text:
                print("[ERROR] The error still occurs!")
            else:
                print("[OK] The error is fixed! The AI gives a helpful response instead of failing.")
        else:
            print(f"[ERROR] Request failed: {response.text}")
        
        if real_task_id:
            print(f"\n--- Testing proper completion with real ID: 'Complete task {real_task_id[:8]}...' ---")
            proper_msg = {"message": f"Complete task {real_task_id}"}
            response = requests.post(f"http://localhost:8000/api/{user_id}/chat", json=proper_msg, headers=headers)
            print(f"Response status: {response.status_code}")
            if response.status_code == 200:
                result = response.json()
                response_text = result.get('response', 'No response')
                print(f"Response: {response_text}")
                
                if "error" in response_text.lower():
                    print("[ERROR] Proper command failed unexpectedly!")
                else:
                    print("[OK] Proper command works correctly!")
            else:
                print(f"[ERROR] Proper command failed: {response.text}")
        
        print(f"\n=== Testing other potentially problematic commands ===")
        
        # Test other commands that might have similar issues
        other_commands = [
            "Complete the task",
            "Update the task",
            "Delete the task",
            "Complete task invalid-id-format",
            "Update task invalid-id-format to have high priority",
            "Delete task invalid-id-format"
        ]
        
        for cmd in other_commands:
            print(f"\n--- Testing: '{cmd}' ---")
            msg = {"message": cmd}
            response = requests.post(f"http://localhost:8000/api/{user_id}/chat", json=msg, headers=headers)
            if response.status_code == 200:
                result = response.json()
                response_text = result.get('response', 'No response')
                print(f"Response: {response_text}")
                
                if "error" in response_text.lower() and ("failed to" in response_text.lower() or "tool execution failed" in response_text.lower()):
                    print(f"[PROBLEM] Still problematic: {cmd}")
                else:
                    print(f"[OK] Handled gracefully: {cmd}")
            else:
                print(f"[ERROR] Request failed: {response.text}")
        
        print(f"\n=== Final verification ===")
        final_msg = {"message": "Show me my tasks"}
        response = requests.post(f"http://localhost:8000/api/{user_id}/chat", json=final_msg, headers=headers)
        if response.status_code == 200:
            result = response.json()
            print(f"Final task list: {result.get('response', 'No response')}")
        else:
            print(f"[ERROR] Final list failed: {response.text}")
        
        print("\n=== Test Complete ===")
        print("The AI Chatbot should now properly handle invalid commands gracefully!")
        
    else:
        print(f"[ERROR] Registration failed: {response.text}")
        
except Exception as e:
    print(f"[ERROR] Error during test: {e}")
    import traceback
    traceback.print_exc()